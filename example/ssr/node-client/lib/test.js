    ...
    if (this.socket) {
      ...
      if (this.config.timeout) {
        timer = setTimeout(function () {
          ...
          callback(
            new Error(
              'request or decode timeout(' + self.config.timeout + 'ms)',
            ),
          )
        }, this.config.timeout)
      }
...
function initSocket(cur) {
  ...
  var connect_timer = setTimeout(function () {
    errorCall(
      new Error('easy_sock:TCP connect timeout(' + connect_timeout + ')'),
    )
    cur.socket = null
...
function tryCloseSocket(cur) {
  if (
    (cur.calling_close || !cur.config.keepAlive) &&
    cur.currentSession == 0 &&
    cur.tmpGetTaskList.length == 0
  ) {
